...
import type { DemoItem, ApiResponse, CalculatorData } from '@shared/types';
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    app.get('/api/test', (c) => c.json({ success: true, data: { name: 'CF Workers Demo' }}));
    // New endpoint for calculator data
    app.get('/api/calculator-data', async (c) => {
        const durableObjectStub = c.env.GlobalDurableObject.get(c.env.GlobalDurableObject.idFromName("global"));
        const data = await durableObjectStub.getCalculatorData();
        return c.json({ success: true, data } satisfies ApiResponse<CalculatorData>);
    });
